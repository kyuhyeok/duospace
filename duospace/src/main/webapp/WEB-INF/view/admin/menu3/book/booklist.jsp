...
$(function(){
	$('#ck_main').click(function(){
		if($(this).prop('checked')){
			$('input[name=signNum]').prop('checked',true);
		}else{
			$('input[name=signNum]').prop('checked', false);
		}
	});
		$("#deletelistBtn").click(function(){
			
			var cnt = $("input[name=signNum]:checkbox:checked").length;
			...
			if(confirm("선택한 항목을 삭제하시겠습니까?")){
				...
				f.action="<%=cp%>/admin/book/deleteList";
				f.submit();
			}
...
function deleteBook(signNum, page) {
	...
	var query = "?signNum="+signNum+"&page="+page;
	location.href = "<%=cp%>/admin/book/delete" + query;
		return;
}
							...
							<form name="selectListForm" class="form-horizontal" method="post">
									...
									<div class="col-sm-2 col-xs-12">
										<select class="form-control" name="searchKey" id="searchform">
											...
											<option value="signNum">등록 번호</option>
					...
					<div class="col-md-12 col-sm-12 col-xs-12">
						<div class="x_panel">
							...
							<div class="x_content">
									...
									<table class="table table-hover" id="table1" style="text-align: center; border-collapse: collapse;">
										...
										<c:forEach var="dto" items="${list}">
											<tr style="text-align: center">
												<td><input class="flat" name="signNum" type="checkbox"
													value="${dto.signNum}" /></td>
												<td>${dto.bookName}</td>
												<td>${dto.writer}</td>
												<td>${dto.publisher}</td>
												<td>${dto.created}</td>
												<td>${dto.status}</td>
												<td>
												<button type="button" class="btn btn-warning btn-sm" onclick="javascript:location.href='<%=cp%>/admin/book/update?signNum=${dto.signNum}&page=${page}';">
												수정
												</button>
												<button type="button" class="btn btn-danger btn-sm"
													onclick="deleteBook(${dto.signNum}, ${page});">
								...
								<div class="form-group">
									<div class="col-xs-12">
										<button type="button" class="btn btn-success btn-sm" onclick="javascript:location.href='<%=cp%>/admin/book/created';">
